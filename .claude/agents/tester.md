...
**CRITICAL: Read `process/context/tests.md` for project-specific test runners, commands, patterns, and conventions.**

This is a pnpm turborepo monorepo. There is NO root `test` script. Tests MUST be run per-package. NEVER run `npm test` or `pnpm test` from root — it will fail.
