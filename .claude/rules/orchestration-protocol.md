...
2. **Reports Path**: `{work_context}/process/context/reports/` for that project
3. **Plans Path**: `{work_context}/process/plans/` for that project

**Example:**
```
Task prompt: "Fix parser bug.
Work context: /path/to/project-b
Reports: /path/to/project-b/process/context/reports/
Plans: /path/to/project-b/process/plans/"
```
...
Plans: [process/plans/ path]
Relevant skills: [comma-separated skill names from Step 0 discovery, if any]
```
...
### Research First for New Service-Shaped Features

When a user proposes adding a new server, daemon, sidecar, agent, or background process, **always route to research-agent first** before INNOVATE or PLAN. Instruct the research-agent to specifically look for:

- Existing analogous services in `packages/api/src/infra/` or `apps/`
- Existing bundle / deploy patterns in `cloud-init.ts`, `index.ts`, or similar infra entry points
- Established conventions for how that type of service is wired up (env, secrets, container run args)

**For features touching VPS host or per-instance Docker operations specifically**: research-agent MUST also inspect `WorkerNode` methods in `packages/api/src/infra/index.ts` to check whether the operation is already implemented as a method on that class (pull, create, start, stop, remove, exec all route through the existing dockerode-over-TLS connection on port 2083). If a `WorkerNode` method already covers the operation, a new host-side service is almost certainly the wrong answer.

**Rationale**: On the VPS host webhook feature (2026-04-25), the plan was spec'd as a vps-api sidecar twice. The operation (`docker pull`) was already available as `WorkerNode.pullLatestImage()` — only found when the user manually inspected the existing code after Phase 1 shipped. The research-first rule existed but didn't fire on the `WorkerNode` layer specifically.
