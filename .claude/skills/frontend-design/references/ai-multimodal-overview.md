# Using Claude's Built-in Multimodal Vision for Frontend Design Assets

Entry point for using Claude's native vision capabilities to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use Claude's built-in multimodal vision in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `ck:frontend-design` with Claude's built-in multimodal vision for asset generation and analysis that follows those guidelines.
...
Post-process generated assets using imagemagick CLI or browser Canvas API: resize, compress, add filters, create compositions.
