...
## Research Phase
```
Task(subagent_type="research-agent", prompt="Research [topic]. Report ≤150 lines.", description="Research [topic]")
```
- Use multiple `research-agent` subagents in parallel for different topics
...
## Planning Phase
```
Task(subagent_type="plan-agent", prompt="Create implementation plan based on reports: [reports]. Save to [path]", description="Plan [feature]")
```
- Input: `research-agent` and scout reports
...
## Parallel Execution
```
Task(subagent_type="execute-agent", prompt="Implement [phase-file] with file ownership: [files]", description="Implement phase [N]")
```
