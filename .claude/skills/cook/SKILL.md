---
name: ck:cook
description: "End-to-end feature implementation alternative to RIPER-5. Use for quick tasks where full Research→Plan→Execute cycle is not needed."
...
> **RIPER-5 Boundary**: `cook` is a standalone alternative to RIPER-5 for quick tasks. When using RIPER-5, do not activate cook — use `ENTER EXECUTE MODE` instead. Cook's internal research/plan steps do NOT create `process/plans/` artifacts by default; add `--save-plan` if you want a persisted plan.
...
- **Finalize (MANDATORY - never skip):**
  1. Spawn `git-manager` for commit/push. Optionally update `process/context/all-context.md` if significant architecture changed.
  2. `TaskUpdate` → mark all Claude Tasks complete after sync-back verification (skip if Task tools unavailable)
  3. Ask user if they want to commit via `git-manager` subagent
  4. Write a concise technical journal entry upon completion

## Required Subagents (MANDATORY)

| Phase | Subagent | Requirement |
|-------|----------|-------------|
| Research | `research-agent` | Optional in fast/code |
| Scout | `ck:scout` | Optional in code |
| Plan | `plan-agent` | Optional in code |
| UI Work | `ui-ux-designer` | If frontend work |
| Testing | `tester`, `debugger` | **MUST** spawn |
| Review | `code-reviewer` | **MUST** spawn |
| Finalize | `git-manager` | Spawn git-manager for commit/push. Optionally update process/context/all-context.md if significant architecture changed. |
