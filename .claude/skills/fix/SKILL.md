---
name: ck:fix
description: "Bug fix workflow: scout → diagnose → fix → verify. Use as standalone alternative for bug fixes, or callable within RIPER-5 EXECUTE phase."
...
- `--parallel` - Activate parallel mode: route to parallel `execute-agent` subagents per issue
...
| **Parallel** | 2+ independent issues OR `--parallel` flag | Parallel `execute-agent` subagents |
...
5. Write a concise technical journal entry upon completion
...
- `innovate-agent` — multiple valid approaches, architecture decision (Deep only)
- `ck:context-engineering` — fixing AI/LLM/agent code

**Subagents:** `debugger`, `research-agent`, `plan-agent`, `code-reviewer`, `tester`, `Bash`
