...
### Parallel (→ multiple execute-agent subagents) — Use Task Trees
...
**Task usage:** Create separate task trees per independent issue (each with scout+diagnose+fix+verify). Spawn `execute-agent` per tree. See `references/task-orchestration.md`.
