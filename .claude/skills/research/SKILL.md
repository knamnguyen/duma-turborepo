...
> **RIPER-5 Integration**: This skill works alongside `research-agent` to provide full-spectrum research capability. `research-agent` handles internal codebase research (read-only, observation-only) and can perform external research via WebSearch. Use this skill (`ck:research`) when you need structured multi-source web research with Gemini CLI support, cross-reference validation, and a saved markdown report. Both can be used within RIPER-5 RESEARCH phase — defer implementation recommendations to INNOVATE phase.
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use the docs-seeker skill to read it.
...
- **Actionability**: Provide clear findings and evidence. Defer specific implementation recommendations to INNOVATE/PLAN phases when within RIPER-5 workflow.
