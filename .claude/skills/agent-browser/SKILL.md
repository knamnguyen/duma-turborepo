...
For Flowser-specific connection patterns (chrome-debug, logged-in session reuse, when to use agent-browser vs chrome-devtools vs docker exec), see `process/context/tests.md` in the consuming repo. This skill file is a generic tool reference only.
