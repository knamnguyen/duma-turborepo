...
> **RIPER-5 Note**: This skill operates as the PLAN phase within RIPER-5. After plan creation, return to the orchestrator and type **ENTER EXECUTE MODE** to continue. Do not route directly to `ck:cook` or other implementation skills — let the RIPER-5 orchestrator control phase transitions.

Create detailed technical implementation plans through research, codebase analysis, solution design, and comprehensive documentation.

**IMPORTANT:** Before you start, scan unfinished plans in the current project at `process/plans/` directory, read the `plan.md`, if there are relevant plans with your upcoming plan, update them as well. If you're unsure or need more clarifications, use `AskUserQuestion` tool to ask the user.
...
| Flag | Mode | Research | Red Team | Validation | Next Step |
|------|------|----------|----------|------------|-----------|
| `--auto` | Auto-detect | Follows mode | Follows mode | Follows mode | Follows mode |
| `--fast` | Fast | Skip | Skip | Skip | ENTER EXECUTE MODE |
| `--hard` | Hard | Deep codebase analysis (no subagents) | Yes | Optional | ENTER EXECUTE MODE |
| `--parallel` | Parallel | Deep codebase analysis (no subagents) | Yes | Optional | ENTER EXECUTE MODE |
| `--two` | Two approaches | Deep codebase analysis (no subagents) | After selection | After selection | ENTER EXECUTE MODE |
...
**Skip if:** Fast mode or provided with research-agent reports
...
flowchart TD
    ...
    D -->|hard/parallel/two| F[Deep Codebase Analysis]
    ...
    L --> M
    M --> N[Output ENTER EXECUTE MODE prompt]
    N --> O[Journal]
```
...
3. **Research Phase** → Perform deep codebase analysis; for external/web research needs, use `ck:research` skill or request `research-agent` from the RIPER-5 orchestrator. Do not spawn subagents directly from within this skill. Skip in fast mode.
4. **Codebase Analysis** → Read docs, scout if needed
5. **Plan Documentation** → Write comprehensive plan via plan-agent
...
9. **Context Reminder** → Inform user: plan is ready, type **ENTER EXECUTE MODE** to begin implementation (MANDATORY — do NOT route to `ck:cook` directly)
