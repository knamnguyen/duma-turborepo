...
2. Use `plan-agent` to create plan
...
1. Spawn max 2 `research-agent` subagents in parallel (different aspects, max 5 calls each)
2. Read codebase docs; if stale/missing: run `/ck:scout` to search codebase
3. Gather research + scout report filepaths → pass to `plan-agent`
