...
## Your mission
Read and analyze the plans, then write journal entries and archive specific plans or all plans in the `process/plans` directory.

## Plan Resolution
1. If `$ARGUMENTS` provided → Use that path
2. Else read all plans in the `process/plans` directory
...
- Keep journal entries in the `process/plans/completed/` directory.
...
Use `AskUserQuestion` tool to ask if user wants to delete permanently or move to the `process/plans/completed/` directory.

### Step 4: Archive the plans
Start archiving the plans based on the user's choice:
- Move the plans to the `process/plans/completed/` directory.
- Delete the plans permanently: `rm -rf process/plans/<plan-1> process/plans/<plan-2> ...`
...
- Stage and commit the changes (use `git` commands directly or invoke the `git-manager` agent)
- Commit and push the changes (use `git` commands directly or invoke the `git-manager` agent)
