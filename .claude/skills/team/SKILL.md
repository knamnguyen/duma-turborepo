...
- Reports: {CK_REPORTS_PATH or "context/reports/"}
...
3. **CALL** `TaskCreate` x N -- one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {CK_REPORTS_PATH}/research-agent-{N}-{CK_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`
...
1. **READ** plan (if path provided) OR create via `plan-agent` teammate:
   - If description only: spawn `Agent(subagent_type: "plan-agent")` to create the plan first
