...
```
# Spawn all 3 research-agent subagents concurrently (non-blocking)
...
Recommended: Most agents get `Task(Explore)` only. `plan-agent` gets `Task(Explore), Task(research-agent)`.
