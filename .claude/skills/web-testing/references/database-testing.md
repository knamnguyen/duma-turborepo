...
> **Project Note (Flowser):** This project uses SQLite/libSQL via Drizzle ORM — not PostgreSQL. For in-project testing, prefer the SQLite in-memory pattern with Drizzle:
> ```typescript
...
> // Run migrations: migrate(db, { migrationsFolder: './drizzle' });
> ```
> The PostgreSQL and MongoDB examples below are kept as generic reference for non-Flowser projects.
